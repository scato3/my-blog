...
export const invalidateAAAQueries = (queryClient: QueryClient) => {
  InvalidateQueries(queryClient, ["aaa", "bbb"]);
};
```
...
- 문자열 배열을 통해 유연하게 쿼리키 전달 가능
- 한 번의 호출로 여러 쿼리키 무효화 가능
...
const handleUpdate = () => {
  InvalidateQueries(queryClient, ["aaa", "bbb", "ccc"]);
};
```

### 4. 확장성

- 필요한 쿼리키를 배열로 자유롭게 전달 가능
- 다른 도메인의 쿼리들도 같은 패턴으로 쉽게 구현 가능

```typescript
// 도메인별로 묶어서 사용 가능
export const invalidateBBBQueries = (queryClient: QueryClient) => {
  InvalidateQueries(queryClient, ["ccc", "ddd", "eee"]);
};
```
