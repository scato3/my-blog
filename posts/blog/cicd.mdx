...
회사에서 Next.js 프로젝트를 운영하면서, 번들 크기가 너무 크고 모노레포 환경에서 배포 시간이 지나치게 오래 걸리는 문제가 있었습니다.
이로 인해 빌드 및 배포 효율을 극대화할 수 있는 구조가 필요하다고 느꼈고, 다양한 캐시 전략과 배포 방식을 적용하게 되었습니다.  
AWS Elastic Beanstalk으로 배포하고, 정적 파일은 S3에 업로드하여 서빙하는 구조를 사용했습니다.  
이 글에서는 standalone 빌드, next/cache, S3 static 서빙, 증분 빌드, 그리고 GitLab CI에서의 캐시 전략까지 실제 적용 사례를 바탕으로 상세히 정리합니다.

## 전체 배포 구조

- **ECR**: Next.js standalone 이미지를 빌드하여 **ECR_REPOSITORY**에 업로드합니다.
...
- 최종 이미지는 **ECR_REPOSITORY**에 업로드됩니다.
...
COPY --link dist/**SERVICE_PATH**/.next/cache/ ./dist/**SERVICE_PATH**/.next/cache/
# ...생략
