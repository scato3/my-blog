...
- 중요한 것은 **모든 값(`a`, `data`, `aRef.current`)이 동일한 값**으로 출력된다는 점입니다. 이는 모두 같은 시점(useEffect 내에서 `a = data`, `aRef.current = data`로 수정된 후)의 값을 캡처하기 때문입니다.
