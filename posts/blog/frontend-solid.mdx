...
- 재사용 불가능 (다른 곳에서 같은 데이터 로직 써야 하면 복사-붙여넣기)
- 변경 범위 예측 불가 (재고 로직 바꾸려다가 렌더링까지 건드림)
...
- Enter/Space 키 동작 안 함 (키보드 사용자 접근 불가)
- Tab으로 포커스 안 됨
- 스크린 리더가 버튼으로 인식 못함 (시각장애인 사용 불가)
- 진짜 **<button>**으로 교체하면 스타일 깨짐 (시맨틱 계약 위반)
...
// 사용
<UserCard user={user} />;
// user.email이 바뀌어도 UserCard가 리렌더됨 (실제로는 안 써도)
```
...
- 단순히 **useQuery** 한 번 감싸는 것 (오버엔지니어링)
- 로직이 5줄 이하고 재사용 안 됨
...
- 모든 컴포넌트에 Polymorphic 적용 (타입 복잡도 상승)
- 접근성 요구사항 없는 내부 툴
...
- 불필요한 리렌더 방지 (name, avatar만 비교)
- 테스트 시 간단한 props만 전달 (`{ name: "test", avatar: "url" }`)
- 재사용성 향상 (User 객체 없어도 사용 가능)
...
1. **300줄 넘는 컴포넌트 찾기**
   ...
   - 데이터 fetching + 변환 로직을 훅으로 분리
   - 한 파일에서 useState/useEffect 5개 넘으면 분리 신호

2. **div 버튼을 button 태그로 교체**

   - **<div onClick>** → **<button>**
   - 키보드 테스트: Tab으로 포커스 → Enter로 동작 확인

3. **거대한 객체 전달 줄이기**
   - UserCard가 User 전체를 받는가? → name, avatar만 받도록
   - props가 5개 넘으면 실제로 다 쓰는지 확인

### 팀에 제안할 것

1. **디자인 시스템 Button에 접근성 체크리스트**

   - 키보드 동작 (Enter/Space)
   - 포커스 스타일
   - ARIA label

2. **필터/정렬이 많은 리스트는 함수 주입 패턴 고려**

   - filterType이 4개 넘으면 filterFn 주입으로 전환
   - 조합 필터 필요하면 함수 주입 필수

3. **SSR 프로젝트는 Storage 인터페이스 도입**
   - localStorage 직접 사용 금지
   - **lib/storage.ts**에 browserStorage/memoryStorage 구현
